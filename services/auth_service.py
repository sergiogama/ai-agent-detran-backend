...
Gerencia login e validação de usuários
"""
...
from .db2_service import Db2Service
...
class AuthService:
    ...
    def __init__(self, db2_service=None):
        if db2_service:
            ...
            # Inicializar com configurações do ambiente usando driver nativo
            self.db2_service = Db2Service(
    ...
    def authenticate_user(self, cpf: str, senha: str) -> Optional[Dict]:
        ...
        """
        try:
            # Tenta buscar com CPF sem formatação
            cpf_limpo = cpf.replace(".", "").replace("-", "")
            condutor = self.db2_service.get_condutor_by_cpf(cpf_limpo)

            # Se não encontrar, tenta com formatação
            if not condutor:
            ...
            if not condutor:
                logger.warning(f"Condutor não encontrado: {cpf_limpo}")
                return None

            # Verifica senha
            if condutor.get("SENHA") != senha:
                logger.warning(f"Senha incorreta para CPF: {cpf_limpo}")
                return None

            logger.info(f"Usuário autenticado com sucesso: {cpf_limpo}")
            return condutor
